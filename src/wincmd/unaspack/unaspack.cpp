...
#define PINAME _T("Aspack Unpacker PlugIn")
#define PIABV _T("<APK>")
...
{
	MessageBox(NULL, _T("Aspack Unpacker PlugIn\nVersion 0.1\n\nGANNIER F."), _T("UnAspack About"), MB_ICONINFORMATION | MB_OK);
}
...
BOOL __stdcall PexPreloadImage(PPGIParamsBlock pPGI)
{
	...
	(pPGI->pCallBack)(id, 0, _T("Executing..."));
	...
	{
//		str.Format(_T("%s: Error in EXE file", PIABV);
//		(pPGI->pCallBack)(id, 0, str);  // 
	...
	{ 
		if (*(NTbase + 2) == 0xE9)
			str.Format(_T("%s: File compressed with Aspack ver 2.11"), PIABV);
		else if (max > 0x3B2)
		{
			if (*(PWORD)(NTbase + 0x3B2) == 0x01B8)
				str.Format(_T("%s: File compressed with Aspack ver 2.12/2.12a/2.12b"), PIABV);
			else if (*(PWORD)(NTbase + 0x3B3) == 0x01B8)
				str.Format(_T("%s: File compressed with Aspack ver ??"), PIABV);
			else if (max > 0x4F8) 
			{
				if (*(PWORD)(NTbase + 0x4F4) == 0x01B8)
					str.Format(_T("%s: File compressed with Aspack 2000"), PIABV);
				else if (*(PWORD)(NTbase + 0x4F6) == 0x01B8)
					str.Format(_T("%s: File compressed with Aspack 2001"), PIABV);
				else if (*(PWORD)(NTbase + 0x4F6) == 0x01B8 && *(NTbase + 0x4DF) == 0x000443A02)
					str.Format(_T("%s: File compressed with Aspack ver 2.1"), PIABV);
				else if (*(PWORD)(NTbase + 0x4F6) == 0x01B8 && *(NTbase + 0x4DF) == 0x0004439FD)
					str.Format(_T("%s: File compressed with Aspack ver ??"), PIABV);
				else if (*(PWORD)(NTbase + 0x3BA) == 0x7561)
					str.Format(_T("%s: File compressed with Aspack ver ??"), PIABV);
				else if (*(NTbase + 2) == 0xE8)
					str.Format(_T("%s: File compressed with Aspack ver 2.11c/d"), PIABV);
				else return FALSE;
			}	else return FALSE;
		}	else return FALSE;
	} else return FALSE;
